...
from DB_setup import Base, Shelter, Puppy, Puppy_Profile
...
for i,x in enumerate(male_names):
	new_puppy = Puppy(name = x, gender = "male", date_of_birth = CreateRandomAge(), shelter_id=randint(1,5), weight= CreateRandomWeight())
	puppy_ID_counter += 1
	new_puppy_profile = Puppy_Profile(puppy_id=puppy_ID_counter, photo=random.choice(puppy_images))
	session.add(new_puppy)
	session.add(new_puppy_profile)
	session.commit()

for i,x in enumerate(female_names):
	new_puppy = Puppy(name = x, gender = "female", date_of_birth = CreateRandomAge(), shelter_id=randint(1,5), weight= CreateRandomWeight())
	puppy_ID_counter += 1
	new_puppy_profile = Puppy_Profile(puppy_id=puppy_ID_counter, photo=random.choice(puppy_images))
	session.add(new_puppy)
	session.add(new_puppy_profile)
